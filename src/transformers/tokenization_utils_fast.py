...
class PreTrainedTokenizerFast(PreTrainedTokenizerBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and tokenizer_object is None and slow_tokenizer is None and fast_tokenizer_file is None:
            if isinstance(args[0], BaseTokenizer):
                tokenizer_object = args[0]
    ...
    def added_tokens_decoder(self) -> Dict[int, AddedToken]:
        ...
        """
        res = {}
        for vocab, id in self._tokenizer.get_vocab().items():
            res[id] = AddedToken(vocab, rstrip=False, lstrip=False, single_word=False, normalized=True)
        return res
